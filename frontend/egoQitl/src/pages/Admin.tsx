...
const Admin: React.FC = () => {
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // Ensure SKU is present (generate if missing for new products)
      const productData = {
        ...productForm,
        sku: productForm.sku || (editingProduct ? editingProduct.sku : `KOP-${Math.floor(Math.random() * 10000)}`)
      };
                  ...
                  <form onSubmit={handleProductSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <Input
                        ...
                        value={productForm.priceRp}
                        onChange={(e) => setProductForm({...productForm, priceRp: parseInt(e.target.value) || 0})}
                        required
                      />
                      <Input
                        type="number"
                      ...
                      {filteredProducts.map((product) => (
                        <tr key={product.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {formatCurrency(product.priceRp)}
                  ...
                  <CardContent className="space-y-3">
                    <div className="text-sm space-y-1">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>{order.deliveryMethod === 'AMBIL' ? 'Ambil' : 'Antar'}</span>
                      </div>
                    </div>
